...
설치 후 Claude Code에서 아래 커맨드를 사용할 수 있습니다.

| 커맨드 | 용도 | 예시 |
|--------|------|------|
| `/ui` | UI 조각 생성 | `/ui 날짜 필터 + 검색 바` |
| `/page` | 전체 페이지 생성 | `/page 사용자 목록 페이지` |
| `/component` | 컴포넌트 props 조회 | `/component Button` |

### 예시

```
/ui 검색 입력 + 필터 칩 + 초기화 버튼
```

```
/page 주문 관리 페이지 (테이블 + 상태 필터 + 페이지네이션)
```
